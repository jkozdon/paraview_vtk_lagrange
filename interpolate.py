def interpolate(Nq, r, w, fld, tmp, nelem):
    fld = reshape(fld, (Nq, Nq, Nq, nelem), order='F')
    h = 2/Nq
...
for e in range(nelem):
    ...
    interpolate(Nq, r, w, x, tmp, 1)
    interpolate(Nq, r, w, y, tmp, 1)
    interpolate(Nq, r, w, z, tmp, 1)
...
for fldname in np_di.PointData.keys():
    ...
    interpolate(Nq, r, w, fld, tmp, size(fld) / (Nq * Nq * Nq))
